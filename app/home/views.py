...
def login():
    ...
    if form.validate_on_submit():  # 如果提交
        ...
        user = (User.query.filter_by(username=data["username"]).first()) or (
            User.query.filter_by(email=data["username"]).first()) or (
                   User.query.filter_by(phone=data["username"]).first())
        if not user:
...
# @home.route('/weibo/',methods=['GET','POST'])
# def bindemail():
...
#             uid = eval(data)['uid']
#             have_u = User.query.filter_by(id=uid).first()
